...
3. Configurar del backend (Supabase)
   3.1 Crear o iniciar sesión en tu cuenta
   3.2 Navegar a https://supabase.com/
   3.3 Clickear en "Start your project" y crear un proyecto nuevo.
   3.4 Crear un archivo con el nombre '.env.local' en el directorio raíz del proyecto y pegar lo siguiente:
   ...
   3.8 En la pestaña de Supabase en la barra desplegable lateral izquierda en el apartadado "SQL Editor"
       ...
       -- Política de Inserción: Permite que el usuario guarde SU propio puntaje
       ...
       -- Política de Lectura: Permite que todos (público) lean los puntajes para armar el ranking
       ...
       -- 5. Vista para acceder rápidamente al ranking 
...
4. OPCIONAL: Desactivar la confirmación de email.
   4.1 Ingresar a "Authentication" en la barra desplegable lateral izquierda en Supabase.
   4.2 En la pestaña lateral "Configuration" ingresar a "Sign In / Providers"
   4.3 En "User Signups" desactivar la opción "Confirm Email" y clickear "Save Changes"

4. Iniciar la app
   ```bash
   npx expo start
   ```
   Una vez iniciado se puede iniciar la app desde Expo Go App desde un teléfono copiando el QR.
   También puede iniciarse mediante web con el link que se genera para 'web'
